...
def build_dataset(src_folder):
    ...
    for i, people_imgs in enumerate(test_people):
        # i = 0,...,n-1 : n = number of people with less than 100 images
        for k in range(5):
            same_pair = random.sample(people_imgs, 2)
                    # randomly sample two image.jpg from same person (people_imgs)
        ...
        for k in range(5):
            ...
            test_set.append((random.choice(test_people[i]), random.choice(test_people[j]), 0))
                    #randomly sample two, one each from different people
...
if __name__ == '__main__':
    ...
    set_to_csv_file(test_set,  test_set_file) #half : two images from same person / half : two images from diff person
